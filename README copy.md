...
```bash
python processing_labeling/fast_detection.py
```
- YOLO detection + Color classification
- Tạo labels YOLO format
- Lưu tại `outputs/labels/`
- Thời gian: ~8-15 phút
...
- Lưu tại `outputs/analytics_charts/`
- Thời gian: ~1-2 phút
...
### 5. Detection & Classification (Dual YOLO, sinh nhãn)
```bash
...
```bash
python scripts/label_data.py --single-yolo   # hoặc bỏ --single-yolo để chạy dual YOLO
```
...
```
- Sau bước này sẽ có file `model_classification.pt`
...
**Lưu ý:**  
- Có thể chạy tự động toàn bộ pipeline (bước 2-7) bằng:
```bash
python scripts/unified_pipeline.py
```
- Nếu chỉ muốn kiểm tra nhanh, có thể bỏ qua bước crawling và filtering bằng các tham số `--skip-crawling`, `--skip-filtering`.
...
│   ├── labeler.py
│   ├── fast_detection.py
│   ├── fast_classify_opencv.py
│   └── master_pipeline.py
├── utils/                     # Utilities
│   ├── database.py           # MinIO + MongoDB
│   ├── logger.py
│   └── performance_monitor.py
├── outputs/                   # Output folders (labels, analytics, compare, sample_images, downloads, logs, ...)
│   ├── labels/
│   ├── labels_n/
│   ├── labels_x/
│   ├── analytics_charts/
...
│   ├── crops/
│   └── logs/
├── datasets/                  # (Nếu cần giữ riêng raw/crops)
│   └── crops/
├── scripts/                   # Script điều phối, pipeline, training, split, UI
...
**Ghi chú:**
- Đặt các file code chính vào các thư mục chức năng:  
  - `config/` cho cấu hình  
  - `data_collection/` cho web scraping  
  - `preprocessing/` cho xử lý ảnh  
  - `processing_labeling/` cho detect, classify, master pipeline  
  - `utils/` cho các tiện ích  
  - `scripts/` cho các script điều phối, training, UI, split, cleanup  
- Tất cả output (labels, analytics, compare, logs, sample_images, downloads, crops, ...) nên gom vào `outputs/` cho gọn.
- Các file dữ liệu lớn, tạm, log, checkpoint, ... KHÔNG để ở root project.
...
```bash
python scripts/view_images.py --view 10 --download
```
...
```bash
python processing_labeling/master_pipeline.py --compare
```

- Ảnh kết quả sẽ được lưu tại thư mục `outputs/compare_results/`
